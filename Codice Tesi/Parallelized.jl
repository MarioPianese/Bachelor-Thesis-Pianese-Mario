...
#CREAZIONE DEL CLOUD
distCloud, minDists = Cloud_Dist_PAR(Cloud);
hmax = maximum(minDists);
Cloud4, Cloud3, Cloud2, Ind_Elim1, Ind_Elim3 = CreateCloud_PAR(Cloud,Normals,tol,alpha);
...
csi, pos, XX, YY, ZZ = Surrounding_Volume_PAR(Cloud2,hmax,lv,beta);

#CALCOLO DELL'INTERPOLANTE
sigma = hmax*k;
Pf = CalcInterpolant_PAR(Cloud4,csi,b,sigma);
